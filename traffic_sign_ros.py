...
def main():
    ...
    image_sub = rospy.Subscriber(IMAGE_TOPIC ,Image,img_callback)
    sign_pub = rospy.Publisher("\sign",String)
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            while True:
                time1 = time.time()
                sign_present = False
                img_blk = np.zeros((ros_img.shape[0], ros_img.shape[1], 3))
                roi_to_classify = None
                if ros_img is None:
                    continue

                process_img_data(ros_img)
                img_clone = ros_img.copy()
                ...
                if to_track == False:
                    ...
                    for box in boxes_filtered:
                        sign_present = True
                        cv2.rectangle(ros_img,(int(box[1] * ros_img.shape[1]),int(box[0] * ros_img.shape[0])),(int(box[3] * ros_img.shape[1]),int(box[2] * ros_img.shape[0])),(0,255,0),3)
                        roi_to_classify = ros_img[int(box[0] * ros_img.shape[0]):int(box[2]*ros_img.shape[0]), int(box[1] * ros_img.shape[0]):int(box[3]*ros_img.shape[0])]

                    if sign_present:
                        print("FOUND TRAFFIC SIGN")
                        bbox = (boxes_filtered[0][1]*ros_img.shape[1], boxes_filtered[0][0]*ros_img.shape[0],
                                (boxes_filtered[0][3] - boxes_filtered[0][1])*ros_img.shape[1],
                                (boxes_filtered[0][2] - boxes_filtered[0][0])*ros_img.shape[0])
                       
                        cv2.imwrite("mar.png",ros_img) 
                        img_clone = cv2.resize(img_clone,(ros_img.shape[1]//2,ros_img.shape[0]//2))
                    ...
                    if ok:
                        print("Ok, tracking")
                        frame = frame + 1
                        roi_to_classify = ros_img[bbox[1]:bbox[1] + bbox[3], bbox[0]:bbox[0] + bbox[2]]
                        cv2.rectangle(ros_img,(bbox[0],bbox[1]),(bbox[0]+bbox[2], bbox[1]+bbox[3]),(0,0,255),3)
                        # try:
                ...
                if roi_to_classify is not None :
                    ...
                    cv2.putText(ros_img, sign_dict[sign], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0 , 0), 2, cv2.LINE_AA)
                    rospy.loginfo(sign_dict[sign])
                    sign_pub.publish(sign_dict[sign])
                ctime = time.time()
                print('FPS: ',1/(ctime - time1))
                cv2.imshow("OUTPUT",ros_img)
                cv2.waitKey(1)
                rospy.spin()
main()
